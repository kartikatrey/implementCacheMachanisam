      ...
      <!-- Graph Header -->
        <el-row type="flex" justify="space-between" class="graph-toolbar-container">
            <el-col :span="16" class="title-container">
                <div class="header">Invoices Processed</div>
            </el-col>    
        </el-row>
    ...
    methods: {
        onClick(selectedData, name) {
            const pushParams = DashboardDrillDownService.getNavigationRoute(
                selectedData,
                "invoice_processed_per_employee",
                null, 
            );
        ...
        async setChartData() {
            let chartResult = await IdpChartService.getChartData({
                chart_name: "InvoiceProcessedWrapper",
                query: this.defaultQuery || {},
            });
            this.setYAxisValues(chartResult);
            //  set X-axis value
             for (let item of chartResult){
                this.option.series[0].data.push({name:item.publishedBy, value: item.total})
            }
            DashboardDrillDownService.setInvoiceProcessedWrapperColor(this.option);
            this.$store.commit('dashboardStore/setdashboardComponentStatus','InvoiceProcessedWrapper');
        },
        async setYAxisValues(chartResult) {
            let values = _.uniq(_.map(chartResult, 'publishedBy'));
            // remove null value 
