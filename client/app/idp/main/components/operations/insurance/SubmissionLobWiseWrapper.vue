<template>
    <div class="submissions-lob-wrapper-container dashboard-cards">
      <!-- Graph Header -->
        <el-row type="flex" justify="space-between" class="graph-toolbar-container">
            <el-col :span="16" class="title-container">
                <div class="header">Submissions LOB Wise</div>
            </el-col>    
        </el-row>
        <stack-bar-chart class="submissions-lob-chart" :name="'submissions-lob-chart'" :options="option" @onClick="onClick"></stack-bar-chart>
    </div>
</template>
    ...
    methods: {
       onClick(selectedData, name) {
            const pushParams = DashboardDrillDownService.getNavigationRoute(
                selectedData,
                "submissions_lobs_wise",
                null
            );
            console.log("DashboardDrillDownService.getNavigationRoute in SubmissionLobWiseWrapper", pushParams);
            this.$router && this.$router.push(pushParams);
        },
        async setChartData() {
            let chartResult = await IdpChartService.getChartData({
                chart_name: "SubmissionLobWiseWrapper",
                query: _.cloneDeep(this.defaultQuery) || {},
            });

            let values = _.uniq(_.map(chartResult, 'lob'));
            ...
            for (let [index,series] of this.option.series.entries()){
                _.map(chartResult, (item)=>{
                    if(this.option.series[index].name == item.stage){
                        this.option.series[index].data.push({name:item.lob, value: item.total})
                    }
                });
            }
            this.$store.commit('dashboardStore/setdashboardComponentStatus','SubmissionLobWiseWrapper');
        },
    },
    beforeMount() {
...
<style lang="scss">
@import "../../../../_idpGlobal";
    .submissions-lob-wrapper-container {
