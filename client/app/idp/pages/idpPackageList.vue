<template>
    <main class="kp-main">
        ...
        <idp-list-header :title="getTitle" :tableProps="tableProps"
            :isPackageType="customerConfiguration.view === 'package'"
            :filter="filter" @onCategoryChange="setCategoryParams"
        ...
        <DocumentSelectList v-if="displayDocumentSelectDialog" :buttonOptions="addToPackageDialogOptions"
            @addDocument="handleAddForm" @closeDialog="closeDialog" :extraQueryParam="extraQueryParams">
            <div style="text-align: center">
                ---------------------------------- OR -------------------------------
            </div>
            <br />
            <PackageForm @closeDialog="closeDialog" :customerConfiguration="customerConfiguration" :document="selectedForm" />
        </DocumentSelectList>
...
export default {
    ...
    data() {
        return {
            ...
            isReviewerOnly: false, // this flage is used to check user role is reviwer only or not
            ...
            tableProps: {
                ...
                tableIdentifier: "idp_" + this.documentType + "_list",
    ...
    computed: {
        ...
        getTitle() {
            const title = _.startCase(this.documentType) || "";
            return title;
        }
    },
    methods: {
        ...
        initColumnFilters(searchText) {
            const payload = { searchText, columns: this.tableProps.columns }
            const columnFilter = DocumentService.getTableColumnFilters(payload);
            this.tableProps.query = { ...columnFilter, ...(this.tableProps.query || {}) };
        },
        ...
        async setExpandColumns() {
            ...
            const columns = columnsData.columns.data;
            let tableProps = _.cloneDeep(this.tableProps);
            tableProps.expandTableParams.parameters.columns = columns;
        ...
        init() {
            ...
            this.tableProps.query.document_type = this.documentType;

            this.isReviewerOnly = _.get(user, "is_reviewer_only") // return true, if user role is revieweronly
            this.$store.commit("setDocumentType", this.documentType);
        ...
        setTableProps() {
            ...
            this.tableProps.query.storage_folder_path = 
                this.filter && this.filter.folderPath ? this.documentType + this.filter.folderPath : undefined

            this.tableProps.query = { ...(this.tableProps.query || {}), ...(this.filter.secondary || {}) };
