...
;(function($) {
    $.fn.fieldsLinker = function (action, input,inv_details,po_details) {  
        console.log("qqq")    
        console.log(input)      
        ...
        if (action == 'init') {
            ...
            factory.work.init(input,inv_details,po_details);
...
function FieldsLinker(selector){
   ...
   this.makeLink = function (infos) {
    ...
    this.makeDropDownForLists = function () {
        var self = this;
        self.dropDownForLists = $('<select disabled="disabled"></select>');
    ...
    this.drawColumnsAtLoadTime = function () {
		...
		if (self.data.options.buttonErase) {
            ...
            $('.eraseLink')
				...
				// .attr("style","position:absolute;bottom:-25px;right:45%;right:45%;padding:0")
    ...
    this.drawColumnsContentA = function () {
        ...
        self.listA.forEach(function (x, i) {
            ...
            var $li = $('<li></li>');
            let item = 
                "<div class='left_side'><div class='description'><span title='"+self.inv_details[i]["description"]+"'>"+self.inv_details[i]["description"]+"</span></div>"
                +"<div class='item_code'> Item Code: "+self.inv_details[i]["item_code"]+"</div>"
                +"<div  class='quantity'>Quantity: "+self.inv_details[i]["quantity"]+"</div>"
                +"<div class='unit_price'>Unit Price: "+self.inv_details[i]["unit_price"]+"</div></div>"
                +"<div class='line_item'>"+self.inv_details[i]["inv_line_number"]+"</div>";
            let id = x;
            $li
                .appendTo(self.$ulLeft)
                .addClass('map_container inv_map_container')
                .attr('data-offset', i)
                .attr('data-name', id)
                .css({
                    'width': '100%',
   ...
   this.drawColumnsContentB = function () {
       var self = this;
        ...
        self.listB.forEach(function (x, i) {
            // let item = self.po_details[i]["po_line_no"]
            let id = x;
            var isMandatory = (self.mandatories.indexOf(x) != -1);
            var $li = $('<li></li>');
            let item = 
            "<div class='line_item'>"+self.po_details[i]["po_line_no"]+"</div>"
                +"<div class='right_side'><div class='description'><span title='"+self.po_details[i]["description"]+"'>"+self.po_details[i]["description"]+"</span></div>"
            +"<div class='item_code'> Item Code: "+self.po_details[i]["item"]+"</div>"
            +"<div  class='quantity'>Quantity: "+self.po_details[i]["balance_quantity"]+"</div>"
            +"<div class='unit_price'>Unit Price: "+self.po_details[i]["unit_price"]+"</div>"
            +"<div class='ship_no'>Ship No: "+self.po_details[i]["shipment_no"]+"</div>"
                + "<div class='supplier_item'> Supplier Item: " + self.po_details[i]["supplier_item"] + "</div></div>";
            // +"<div class='line_item'>"+(i+1)+"</div>";
            $li
                .appendTo(self.$ulRight)
                .addClass('map_container po_map_container')
                .attr('data-offset', i)
                .attr('data-name', id)
    ...
    this.setListeners = function () {
        var self = this;
        if (self.data.options.buttonErase) {
            $(self.$root).find('.eraseLink').on('click', function (e) {
                if (self.isDisabled) return;
    ...
    this.init = function(input,inv_details,po_details,onFilter){
